 ...
 *
 * @param {string} sessionId - The unique ID of the target session to cancel.
 * @param {string} [targetDir] - Optional directory context to resolve API credentials.
 * @returns {Promise<any>} A promise resolving to the API response object.
 ...
 *
 * @param {string} sessionId - The unique ID of the target session.
 * @param {string} message - The message text to post to the session.
 * @param {string} [targetDir] - Optional directory context to resolve API credentials.
 * @returns {Promise<any>} A promise resolving to the API response object.
 * @throws {Error} Throws an error if API key is missing or request fails.
 */
export async function sendMessageApi(sessionId: string, message: string, targetDir?: string): Promise<any> {
  ...
  if (!apiKey) throw new Error('JULES_API_KEY not found in environment or .env file.');
  const headers = { 'X-Goog-Api-Key': apiKey };
  return await request(`https://jules.googleapis.com/v1alpha/sessions/${sessionId}:sendMessage`, {
    method: 'POST',
    headers
  }, { prompt: message });
}
 ...
 *
 * @param {string} [targetDir] - Optional directory context to resolve API credentials.
 * @returns {Promise<JulesSource[]>} A promise resolving to an array of Jules sources.
 * @throws {Error} Throws an error if API key is missing or request fails.
 */
 ...
 *
 * @param {string} sessionId - The target session ID to extract patch from.
 * @param {string} [targetDir] - Optional directory context to resolve API credentials.
 * @returns {Promise<string>} A promise resolving to the extracted unidiff patch string.
